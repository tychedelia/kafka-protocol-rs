...
use kafka_protocol::{messages::{fetch_response, FetchResponse}, records::RecordBatchDecoder};
...
const SECOND_RECORD: [u8; 79] = [
    ...
    0x2,
    // Key
    0xa, 0x68, 0x69, 0x69, 0x69, 0x69,
    // Value
    0xa, 0x62, 0x79, 0x65, 0x65, 0x65,
    // Header
    0x0,
];
    ...
    for topic in res.responses {
        ...
        for partition in topic.partitions {
            ...
            for record in records {
                assert_eq!(String::from_utf8(record.key.unwrap().to_vec()).unwrap(), "hello");
                assert_eq!(String::from_utf8(record.value.unwrap().to_vec()).unwrap(), "world");
            }
        }
    }
}
...
fn second_record() {
    ...
    for topic in res.responses {
        ...
        for partition in topic.partitions {
            ...
            assert_eq!(records.len(), 1);
            for record in records {
                assert_eq!(String::from_utf8(record.key.unwrap().to_vec()).unwrap(), "hiiii");
                assert_eq!(String::from_utf8(record.value.unwrap().to_vec()).unwrap(), "byeee");
            }
        }
    }
}
